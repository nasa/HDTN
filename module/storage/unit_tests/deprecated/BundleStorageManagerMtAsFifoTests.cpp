/**
 * @file BundleStorageManagerMtAsFifoTests.cpp
...
BOOST_AUTO_TEST_CASE(BundleStorageManagerAllAsFifoTestCase)
{
    for (unsigned int whichBsm = 0; whichBsm < 2; ++whichBsm) {
        ...
        if (whichBsm == 0) {
            std::cout << "create BundleStorageManagerMT for FIFO" << std::endl;
            bsmPtr = boost::make_unique<BundleStorageManagerMT>(ptrStorageConfig);
        }
        else {
            std::cout << "create BundleStorageManagerAsio for FIFO" << std::endl;
...
BOOST_AUTO_TEST_CASE(BundleStorageManagerAllAsFifo_RestoreFromDisk_TestCase)
{
    ...
    for (unsigned int whichBsm = 0; whichBsm < 2; ++whichBsm) {
        ...
        { //scope this bsm instance (deleted when going out of scope)
            ...
            if (whichBsm == 0) {
                std::cout << "create BundleStorageManagerMT for FIFO MODE Restore" << std::endl;
                bsmPtr = boost::make_unique<BundleStorageManagerMT>(ptrStorageConfig);
            }
            else {
                std::cout << "create BundleStorageManagerAsio for FIFO MODE Restore" << std::endl;
                bsmPtr = boost::make_unique<BundleStorageManagerAsio>(ptrStorageConfig);
            }
            BundleStorageManagerBase & bsm = *bsmPtr;
...

        {
            ...
            BOOST_REQUIRE_EQUAL(bsm.m_totalBundlesRestored, BUNDLES_TO_SEND);
            bsm.Start();
