...
BOOST_AUTO_TEST_CASE(BundleStorageManagerAllTestCase)
{
    for (unsigned int whichBsm = 0; whichBsm < 2; ++whichBsm) {
        ...
        if (whichBsm == 0) {
            ...
            bsmPtr = boost::make_unique<BundleStorageManagerMT>((Environment::GetPathHdtnSourceRoot() / "tests" / "config_files" / "storage" / "storageConfigRelativePaths.json").string());
        }
        else {
            std::cout << "create BundleStorageManagerAsio" << std::endl;
            bsmPtr = boost::make_unique<BundleStorageManagerAsio>((Environment::GetPathHdtnSourceRoot() / "tests" / "config_files" / "storage" / "storageConfigRelativePaths.json").string());
        }
...
BOOST_AUTO_TEST_CASE(BundleStorageManagerMt_RestoreFromDisk_TestCase)
{
    boost::random::mt19937 gen(static_cast<unsigned int>(std::time(0)));
    const boost::random::uniform_int_distribution<> distRandomData(0, 255);
    const boost::random::uniform_int_distribution<> distPriorityIndex(0, 2);
...

    {
        ...
        BundleStorageManagerMT bsm((Environment::GetPathHdtnSourceRoot() / "tests" / "config_files" / "storage" / "storageConfigRelativePaths.json").string());
        bsm.Start(false);
