...
BundleUuidToUint64HashMap::BundleUuidToUint64HashMap() : m_bucketsPtr(boost::make_unique<bucket_array_t>()), m_buckets(*m_bucketsPtr) {}


BundleUuidToUint64HashMap::~BundleUuidToUint64HashMap() {}


uint16_t BundleUuidToUint64HashMap::GetHash(const cbhe_bundle_uuid_t & bundleUuid) {
...
bool BundleUuidToUint64HashMap::Insert(const cbhe_bundle_uuid_t & bundleUuid, uint64_t value) {
    return Insert(GetHash(bundleUuid), bundleUuid, value);
}

//insert into buckets in order
//return true if inserted, false if already exists
bool BundleUuidToUint64HashMap::Insert(const uint16_t hash, const cbhe_bundle_uuid_t & bundleUuid, uint64_t value) {
    ...
    if (bucket.empty()) {
        ...
        for (bucket_t::const_iterator it = bucket.cbegin(); it != bucket.cend(); ++it, ++itPrev) {
            const cbhe_bundle_uuid_t & bundleUuidInList = it->first;
...
bool BundleUuidToUint64HashMap::GetValueAndRemove(const cbhe_bundle_uuid_t & bundleUuid, uint64_t & value) {
    return GetValueAndRemove(GetHash(bundleUuid), bundleUuid, value);
}

//return true if exists, false if key doesn't exist in the map
bool BundleUuidToUint64HashMap::GetValueAndRemove(const uint16_t hash, const cbhe_bundle_uuid_t & bundleUuid, uint64_t & value) {
    bucket_t & bucket = m_buckets[hash];
    if (bucket.empty()) {
        return false;
    }
...
void BundleUuidToUint64HashMap::Clear() {
    for (std::size_t i = 0; i < m_buckets.size(); ++i) {
        m_buckets[i].clear();
    }
}

void BundleUuidToUint64HashMap::BucketToVector(const uint16_t hash, std::vector<pair_uuid_uint64_t> & bucketAsVector) {
...
std::size_t BundleUuidToUint64HashMap::GetBucketSize(const uint16_t hash) {
    bucket_t & bucket = m_buckets[hash];
    std::size_t size = 0;
    for (bucket_t::const_iterator it = bucket.cbegin(); it != bucket.cend(); ++it) {
        ++size;
