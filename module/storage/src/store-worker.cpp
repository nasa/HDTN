...
static void* _launch_wrapper(void* arg) {
    hdtn3::storage_worker* worker = (hdtn3::storage_worker*) arg;
...
void* hdtn3::storage_worker::execute(void* arg) {
    zmq::message_t rhdr;
    zmq::message_t rmsg;
    std::cout << "[storage-worker] Worker thread starting up." << std::endl;
    _out_buf = (char *)malloc(HDTN3_BLOSC_MAXBLOCKSZ);
    ...
    if(!_store.init(_root)) {
        startup_notify.type = HDTN3_MSGTYPE_IABORT;
    ...
    while(true) {
        ...
        if(rhdr.size() < sizeof(hdtn3::common_hdr)) {
            std::cerr << "[storage-worker] Invalid message format - header size too small (" << rhdr.size() << ")" << std::endl;
            continue;
        }
        hdtn3::common_hdr* common = (hdtn3::common_hdr*)rhdr.data();
        if(common->type == HDTN3_MSGTYPE_STORE) {
            _worker_sock.recv(&rmsg);
            hdtn3::block_hdr* block = (hdtn3::block_hdr*)rhdr.data();
            if(rhdr.size() != sizeof(hdtn3::block_hdr)) {
...
void hdtn3::storage_worker::write(hdtn3::block_hdr* hdr, zmq::message_t* message) {
    // std::cerr << "[storage-worker] Received chunk of size " << rmsg.size() << std::endl;
    uint64_t chunks = ceil(message->size() / (double)HDTN3_BLOSC_MAXBLOCKSZ);
    for(int i = 0; i < chunks; ++i) {
        int res = blosc_compress_ctx(9, 0, 4, message->size(), message->data(), _out_buf, HDTN3_BLOSC_MAXBLOCKSZ, "lz4", 0, 1);
...
void hdtn3::storage_worker::launch() {
