...
int main(int argc, char* argv[]) {
    ...
    uint64_t number_to_send = 1000000;
    ...
    uint64_t bseq = 0;
    fprintf(log, "elapsed, bundle count, raw bytes,bundle data,total bytes\n");
    fflush(log);
    while (true) {
        int idx = 0;
        for (idx = 0; idx < BP_MSG_NBUF; ++idx) {
            ...
            if (curr_time == last_time) {
                ...
                elapsed -= start;
                start = start + elapsed;
                fprintf(log, "%0.6f, %lu, %lu, %lu, %lu,\n", elapsed, bundle_count, raw_data, bundle_data, total_bytes);
                fflush(log);
                bundle_count = 0;
                bundle_data = 0;
                total_bytes = total_bytes + raw_data;
