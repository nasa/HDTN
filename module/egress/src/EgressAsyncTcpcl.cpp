...
m_keepAliveIntervalSeconds(30), //todo, make this configurable
...
m_totalBundleBytesSent(0)
{
    ...
    m_tcpcl.SetContactHeaderReadCallback(boost::bind(&HegrTcpclEntryAsync::ContactHeaderCallback, this, boost::placeholders::_1, boost::placeholders::_2, boost::placeholders::_3));
    m_tcpcl.SetDataSegmentContentsReadCallback(boost::bind(&HegrTcpclEntryAsync::DataSegmentCallback, this, boost::placeholders::_1, boost::placeholders::_2, boost::placeholders::_3));
    m_tcpcl.SetAckSegmentReadCallback(boost::bind(&HegrTcpclEntryAsync::AckCallback, this, boost::placeholders::_1));
    m_tcpcl.SetBundleRefusalCallback(boost::bind(&HegrTcpclEntryAsync::BundleRefusalCallback, this, boost::placeholders::_1));
    m_tcpcl.SetNextBundleLengthCallback(boost::bind(&HegrTcpclEntryAsync::NextBundleLengthCallback, this, boost::placeholders::_1));
    m_tcpcl.SetKeepAliveCallback(boost::bind(&HegrTcpclEntryAsync::KeepAliveCallback, this));
    m_tcpcl.SetShutdownMessageCallback(boost::bind(&HegrTcpclEntryAsync::ShutdownCallback, this, boost::placeholders::_1, boost::placeholders::_2, boost::placeholders::_3, boost::placeholders::_4));
...
int HegrTcpclEntryAsync::Forward(boost::shared_ptr<zmq::message_t> zmqMessagePtr) {
    ...
    m_totalBundleBytesSent += zmqMessagePtr->size();
    m_bytesToAckQueue.push(zmqMessagePtr->size());

    boost::shared_ptr<std::vector<uint8_t> > bundleSegmentPtr = boost::make_shared<std::vector<uint8_t> >();
    Tcpcl::GenerateDataSegment(*bundleSegmentPtr, true, true, (const uint8_t*)zmqMessagePtr->data(), static_cast<uint32_t>(zmqMessagePtr->size()));

    boost::asio::async_write(*m_tcpSocketPtr, boost::asio::buffer(*bundleSegmentPtr),
                                     boost::bind(&HegrTcpclEntryAsync::HandleTcpSend, this, bundleSegmentPtr,
...
void HegrTcpclEntryAsync::Connect(const std::string & hostname, const std::string & port) {
    ...
    m_resolver.async_resolve(query, boost::bind(&HegrTcpclEntryAsync::OnResolve,
                                                this, boost::asio::placeholders::error,
                                                boost::asio::placeholders::results));
}

void HegrTcpclEntryAsync::OnResolve(const boost::system::error_code & ec, boost::asio::ip::tcp::resolver::results_type results) { // Resolved endpoints as a range.
    if(ec) {
        ...
        boost::asio::async_connect(
            *m_tcpSocketPtr,
            results,
            boost::bind(
                &HegrTcpclEntryAsync::OnConnect,
                this,
                boost::asio::placeholders::error));
    }
}

void HegrTcpclEntryAsync::OnConnect(const boost::system::error_code & ec) {
    ...
    if(m_tcpSocketPtr) {
        ...
        Tcpcl::GenerateContactHeader(*contactHeaderPtr, CONTACT_HEADER_FLAGS::REQUEST_ACK_OF_BUNDLE_SEGMENTS, m_keepAliveIntervalSeconds, "egress");
        boost::asio::async_write(*m_tcpSocketPtr, boost::asio::buffer(*contactHeaderPtr),
                                         boost::bind(&HegrTcpclEntryAsync::HandleTcpSend, this, contactHeaderPtr,
                                                     boost::asio::placeholders::error,
                                                     boost::asio::placeholders::bytes_transferred, false));
...
void HegrTcpclEntryAsync::ShutdownAndCloseTcpSocket() {
    if(m_tcpSocketPtr) {
...
void HegrTcpclEntryAsync::HandleTcpSend(boost::shared_ptr<std::vector<boost::uint8_t> > dataSentPtr, const boost::system::error_code& error, std::size_t bytes_transferred, bool closeSocket) {
    if(error) {
...
void HegrTcpclEntryAsync::StartTcpReceive() {
    m_tcpSocketPtr->async_read_some(
        boost::asio::buffer(m_tcpReadSomeBuffer, 2000),
        boost::bind(&HegrTcpclEntryAsync::HandleTcpReceiveSome, this,
                    boost::asio::placeholders::error,
                    boost::asio::placeholders::bytes_transferred));
}
void HegrTcpclEntryAsync::HandleTcpReceiveSome(const boost::system::error_code & error, std::size_t bytesTransferred) {
    if (!error) {
        //std::cout << "received " << bytesTransferred << "\n";

        //because egress will not receive much data from the destination,
...
void HegrTcpclEntryAsync::ContactHeaderCallback(CONTACT_HEADER_FLAGS flags, uint16_t keepAliveIntervalSeconds, const std::string & localEid) {
    ...
    if(m_keepAliveIntervalSeconds) { //non-zero
        ...
        m_noKeepAlivePacketReceivedTimer.async_wait(boost::bind(&HegrTcpclEntryAsync::OnNoKeepAlivePacketReceived_TimerExpired, this, boost::asio::placeholders::error));


        m_needToSendKeepAliveMessageTimer.expires_from_now(boost::posix_time::seconds(m_keepAliveIntervalSeconds));
        m_needToSendKeepAliveMessageTimer.async_wait(boost::bind(&HegrTcpclEntryAsync::OnNeedToSendKeepAliveMessage_TimerExpired, this, boost::asio::placeholders::error));
    }

}

void HegrTcpclEntryAsync::DataSegmentCallback(boost::shared_ptr<std::vector<uint8_t> > dataSegmentDataSharedPtr, bool isStartFlag, bool isEndFlag) {

    std::cout << "egress should never enter DataSegmentCallback" << std::endl;
}

void HegrTcpclEntryAsync::AckCallback(uint32_t totalBytesAcknowledged) {
...
void HegrTcpclEntryAsync::BundleRefusalCallback(BUNDLE_REFUSAL_CODES refusalCode) {
    std::cout << "error: BundleRefusalCallback not implemented yet in egress" << std::endl;
}

void HegrTcpclEntryAsync::NextBundleLengthCallback(uint32_t nextBundleLength) {
    std::cout << "egress should never enter NextBundleLengthCallback" << std::endl;
}

void HegrTcpclEntryAsync::KeepAliveCallback() {
...
void HegrTcpclEntryAsync::ShutdownCallback(bool hasReasonCode, SHUTDOWN_REASON_CODES shutdownReasonCode,
...
void HegrTcpclEntryAsync::OnNoKeepAlivePacketReceived_TimerExpired(const boost::system::error_code& e) {
    if (e != boost::asio::error::operation_aborted) {
        ...
        if(m_tcpSocketPtr) {
            ...
            boost::asio::async_write(*m_tcpSocketPtr, boost::asio::buffer(*shutdownPtr),
                                             boost::bind(&HegrTcpclEntryAsync::HandleTcpSend, this, shutdownPtr,
                                                         boost::asio::placeholders::error,
...
void HegrTcpclEntryAsync::OnNeedToSendKeepAliveMessage_TimerExpired(const boost::system::error_code& e) {
    if (e != boost::asio::error::operation_aborted) {
        // Timer was not cancelled, take necessary action.
        ...
        if(m_tcpSocketPtr) {
            ...
            boost::asio::async_write(*m_tcpSocketPtr, boost::asio::buffer(*keepAlivePtr),
                                             boost::bind(&HegrTcpclEntryAsync::HandleTcpSend, this, keepAlivePtr,
                                                         boost::asio::placeholders::error,
                                                         boost::asio::placeholders::bytes_transferred, false));
