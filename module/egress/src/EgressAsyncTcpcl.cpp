...
int HegrTcpclEntryAsync::Forward(std::unique_ptr<zmq::message_t> && zmqMessagePtr, unsigned int & numUnackedBundles) {
    if (!(m_flags & HEGR_FLAG_UP)) {
        return 0;
    }
    if(m_tcpclBundleSourcePtr && m_tcpclBundleSourcePtr->Forward(std::move(zmqMessagePtr), numUnackedBundles)) {
        return 1;
