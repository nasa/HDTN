...
BOOST_AUTO_TEST_CASE(TelemetryRunnerProgramOptionsDefaultTestCase)
{
    ...
    std::string defaultRoot = (Environment::GetPathHdtnSourceRoot() / "module" / "telem_command_interface" / "src" / "gui" ).string();
...
{
    boost::program_options::variables_map vm;
    std::string validRoot = (Environment::GetPathHdtnSourceRoot() / "module" / "telem_command_interface" / "src" / "gui" ).string();
