...
#define ingress_adopt_lock boost::adopt_lock
    ...
    Ingress::Impl::BundlePipelineAckingSet::BundlePipelineAckingSet(const uint64_t paramMaxBundlesInPipeline,
                                                                    const uint64_t paramMaxBundleSizeBytesInPipeline, const uint64_t paramNextHopNodeId, bool paramLinkIsUp) : m_egressBytesInPipeline(0),
    ...
    bool Ingress::Impl::BundlePipelineAckingSet::CompareAndPop_ThreadSafe(const uint64_t uniqueId, const bool isEgress)
    {
        ...
        umap_64_to_64_t &mapBundleUniqueIdToBundleSizeBytes = (isEgress) ? m_mapEgressBundleUniqueIdToBundleSizeBytes : m_mapStorageBundleUniqueIdToBundleSizeBytes;
                                                                                       ...
                                                                                       bool &reservedEgressPipelineAvailability, bool &reservedStoragePipelineAvailability)
    {
        ...
        {
            reservedStoragePipelineAvailability = (m_mapStorageBundleUniqueIdToBundleSizeBytes.size() < halfOfMaxBundlesInPipeline) && ((m_storageBytesInPipeline + bundleSizeBytes) <= halfOfMaxBytesInPipeline);
    ...
    Ingress::Impl::Impl() : m_bundleCountStorage(0),
    ...
    Ingress::Ingress() : m_pimpl(boost::make_unique<Ingress::Impl>()),
                         // references
                             ...
                             const HdtnDistributedConfig &hdtnDistributedConfig, zmq::context_t *hdtnOneProcessZmqInprocContextPtr)
    {
        ...
        M_HDTN_EID_TO_ROUTER_BUNDLES.Set(m_hdtnConfig.m_myNodeId, m_hdtnConfig.m_myRouterServiceId);
        ...
        m_zmqSubSock_boundRouterToConnectingIngressPtr = boost::make_unique<zmq::socket_t>(*m_zmqCtxPtr, zmq::socket_type::sub);
        ...
        try
        {
            m_zmqSubSock_boundRouterToConnectingIngressPtr->connect(connect_boundRouterPubSubPath);
            m_zmqSubSock_boundRouterToConnectingIngressPtr->set(zmq::sockopt::linger, 0); // prevent hang when deleting the zmqCtxPtr
        ...
        try
        {
            ...
            m_zmqSubSock_boundRouterToConnectingIngressPtr->set(zmq::sockopt::subscribe, "aa");
        ...
        { // start worker thread
            ...
            while (m_workerThreadStartupInProgress)
            { // lock mutex (above) before checking condition
        ...
        { // keep thread alive if running
            ...
            { // try to gain exclusive access
                ...
                else
                {
                    ...
                    if ((!foundError) && (!m_egressFullyInitialized))
                    {                                                                                      // first time this outduct capabilities telemetry received, start remaining ingress threads
            ...
            if (rc > 0)
            {
                ...
                { // ack from egress
                    ...
                    else if (receivedEgressAckHdr.base.type == HDTN_MSGTYPE_EGRESS_ACK_TO_INGRESS)
                    {
                        ...
                        if (bundlePipelineAckingSetObj.CompareAndPop_ThreadSafe(receivedEgressAckHdr.custodyId, true))
                        { // true => isEgress
                ...
                { // ack from storage
                    ...
                    {
                        ingress_shared_lock_t lockShared(m_sharedMutexFinalDestsToOutductArrayIndexMaps);
                        BundlePipelineAckingSet &bundlePipelineAckingSetObj = (receivedStorageAck.outductIndex == UINT64_MAX) ? m_singleStorageBundlePipelineAckingSet : (*(m_vectorBundlePipelineAckingSet[receivedStorageAck.outductIndex]));
                        ...
                        if (bundlePipelineAckingSetObj.CompareAndPop_ThreadSafe(receivedStorageAck.ingressUniqueId, false))
                        { // false => is Storage
                            bundlePipelineAckingSetObj.NotifyAll();
        ...
        { // keep thread alive if running
            ...
            if ((rc > 0) && (items[0].revents & ZMQ_POLLIN))
            {
                ...
                else
                {
                    ...
                    if (messageFlags)
                    { // 1 => from egress and needs processing (is padded from the convergence layer)
                    ...
                    { // 0 => from storage and needs no processing (is not padded)
    ...
    void Ingress::Impl::RouterEventHandler()
    {
        hdtn::IreleaseChangeHdr releaseChangeHdr;
        const zmq::recv_buffer_result_t res = m_zmqSubSock_boundRouterToConnectingIngressPtr->recv(zmq::mutable_buffer(&releaseChangeHdr, sizeof(releaseChangeHdr)), zmq::recv_flags::none);
        if (!res)
        {
        ...
        else if (releaseChangeHdr.base.type == HDTN_MSGTYPE_BUNDLES_FROM_ROUTER)
        {
            std::unique_ptr<zmq::message_t> zmqMessageBundleFromRouterPtr = boost::make_unique<zmq::message_t>();
            // message guaranteed to be there due to the zmq::send_flags::sndmore
            if (!m_zmqSubSock_boundRouterToConnectingIngressPtr->recv(*zmqMessageBundleFromRouterPtr, zmq::recv_flags::none))
            {
                                          ...
                                          const bool usingZmqData, const bool needsProcessing, const bool isSafeToYieldThisThread)
    {
        ...
        if (bundleCurrentSize > m_hdtnConfig.m_maxBundleSizeBytes)
        { // should never reach here as this is handled by induct
        ...
        else if (isBpVersion7)
        {
            ...
            if (!bv.LoadBundle(bundleDataBegin, bundleCurrentSize, skipCrcVerifyInCanonicalBlocks))
            { // todo true => skip canonical block crc checks to increase speed
       ...
       // get previous node
                    ...
                    { // prepend new previous node block
        ...
        { // forward to egress which will forward to router
            ...
            toEgressHdr->base.type = HDTN_MSGTYPE_BUNDLES_TO_ROUTER;
            {
                ...
                else
                {
                    ...
                    {
                        LOG_ERROR(subprocess) << "can't send bundle intended for router to egress";
                    }
                    else
                    {                                                 // success
        ...
        */
        ...
        {
            if (tcpclInductIterator->second->ForwardOnOpportunisticLink(finalDestEid.nodeId, *zmqMessageToSendUniquePtr, 3))
            { // thread safe forward with 3 second timeout
        ...
        if (!sentDataOnOpportunisticLink)
        {
            ...
            { // begin scope for cut-through shared mutex lock
                ...
                if (outductIndex != UINT64_MAX)
                {
                    ...
                    { // type egress cut through ("while loop" instead of "if statement" to support breaking to storage)
                        ...
                        const boost::posix_time::time_duration &cutThroughTimeoutRef = (m_hdtnConfig.m_bufferRxToStorageOnLinkUpSaturation)
                                                                                           ? noDuration
                                                                                           : M_MAX_INGRESS_BUNDLE_WAIT_ON_EGRESS_TIME_DURATION;
                        ...
                        if (foundACutThroughPath)
                        {
                            ...
                            { // pipeline limit exceeded for egress cut-through path
                                useStorage = true;
                            ...
                            { // if(reservedEgressPipelineAvailability) //pipeline limits not exceeded for egress cut-through path, continue to send the bundle to egress
                        ...
                        { // did not find a cut through path
                            useStorage = true;
                ...
                { // storage
                    ...
                    if (!reservedStorageCutThroughPipelineAvailability)
                    { // cut through path was not available for egress or storage, time to store the bundle
                    ...
                    if (storageModuleAvailable)
                    {
                        ...
                        BundlePipelineAckingSet &ackingSetObj = (outductIndex == UINT64_MAX) ? m_singleStorageBundlePipelineAckingSet : (*(m_vectorBundlePipelineAckingSet[outductIndex])); // only used to restore state if zmq fails
