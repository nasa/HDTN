...
timeout /t 4
START "BpGen" /D "%HDTN_BUILD_ROOT%" "cmd /k" "%HDTN_BUILD_ROOT%\common\bpcodec\apps\bpgen-async.exe" "--bundle-rate=100" "--my-uri-eid=ipn:1.1" "--dest-uri-eid=ipn:2.1" "--cla-rate=500000" "--outducts-config-file=%HDTN_SOURCE_ROOT%\config_files\outducts\bpgen_one_udp_port4556_0.5Mbps.json"
