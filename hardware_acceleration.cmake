...
if(USE_X86_HARDWARE_ACCELERATION)
	...
	if(USE_SDNV_FAST AND SSE_supported AND SSE2_supported AND SSE3_supported AND SSSE3_supported AND SSE41_supported AND POPCNT_supported AND BMI1_supported AND BMI2_supported)
		...
		list(APPEND COMPILE_DEFINITIONS_TO_EXPORT USE_SDNV_FAST) #used in Sdnv.h (should be fixed, but for now the package config must export it)
		list(APPEND NON_WINDOWS_HARDWARE_ACCELERATION_FLAGS -msse -msse2 -msse3 -mssse3 -msse4.1 -mbmi -mbmi2)
	endif()
	if(SDNV_SUPPORT_AVX2_FUNCTIONS AND AVX_supported AND AVX2_supported AND USE_SDNV_FAST)
		...
		list(APPEND COMPILE_DEFINITIONS_TO_EXPORT SDNV_SUPPORT_AVX2_FUNCTIONS) #used in Sdnv.h (should be fixed, but for now the package config must export it)
		list(APPEND NON_WINDOWS_HARDWARE_ACCELERATION_FLAGS -mavx -mavx2)
	endif()
	if(USE_CBOR_FAST AND SSE_supported AND SSE2_supported)
		...
		list(APPEND NON_WINDOWS_HARDWARE_ACCELERATION_FLAGS -msse -msse2)
	endif()
	if(USE_CRC32C_FAST AND SSE42_supported)
		...
		list(APPEND NON_WINDOWS_HARDWARE_ACCELERATION_FLAGS -msse4.2)
	endif()
	if(USE_ANDN AND BMI1_supported)
		...
		list(APPEND NON_WINDOWS_HARDWARE_ACCELERATION_FLAGS -mbmi)
	endif()
	if(USE_BITTEST)
		message("adding compile definition: USE_BITTEST")
		add_compile_definitions(USE_BITTEST)
	endif()
	list(REMOVE_DUPLICATES NON_WINDOWS_HARDWARE_ACCELERATION_FLAGS)
endif()
