...
bool Bpv7BlockIntegrityBlock::AddOrUpdateSecurityParameterShaVariant(COSE_ALGORITHMS alg) {
    return Protected_AddOrUpdateUintValueSecurityParameter(
        static_cast<parameter_id_t>(BPSEC_BIB_HMAX_SHA2_SECURITY_PARAMETERS::SHA_VARIANT),
        static_cast<uint64_t>(alg));
}
COSE_ALGORITHMS Bpv7BlockIntegrityBlock::GetSecurityParameterShaVariant(bool & success) const {
    return static_cast<COSE_ALGORITHMS>(Protected_GetUintSecurityParameter(static_cast<parameter_id_t>(BPSEC_BIB_HMAX_SHA2_SECURITY_PARAMETERS::SHA_VARIANT), success));
}
BPSEC_BIB_HMAX_SHA2_INTEGRITY_SCOPE_MASKS Bpv7BlockIntegrityBlock::GetSecurityParameterScope() const {
    bool success;
    BPSEC_BIB_HMAX_SHA2_INTEGRITY_SCOPE_MASKS scope = static_cast<BPSEC_BIB_HMAX_SHA2_INTEGRITY_SCOPE_MASKS>(
        Protected_GetUintSecurityParameter(static_cast<parameter_id_t>(BPSEC_BIB_HMAX_SHA2_SECURITY_PARAMETERS::INTEGRITY_SCOPE_FLAGS), success));
    if (!success) {
        ...
        scope = BPSEC_BIB_HMAX_SHA2_INTEGRITY_SCOPE_MASKS::ALL_FLAGS_SET;
    }
    return scope;
}
bool Bpv7BlockIntegrityBlock::AddSecurityParameterIntegrityScope(BPSEC_BIB_HMAX_SHA2_INTEGRITY_SCOPE_MASKS integrityScope) {
    return Protected_AddOrUpdateUintValueSecurityParameter(
        static_cast<parameter_id_t>(BPSEC_BIB_HMAX_SHA2_SECURITY_PARAMETERS::INTEGRITY_SCOPE_FLAGS),
...
std::vector<uint8_t>* Bpv7BlockIntegrityBlock::AddAndGetAesWrappedKeyPtr() {
    return Protected_AddAndGetByteStringParamPtr(static_cast<parameter_id_t>(BPSEC_BIB_HMAX_SHA2_SECURITY_PARAMETERS::WRAPPED_KEY));
}
std::vector<uint8_t>* Bpv7BlockIntegrityBlock::GetAesWrappedKeyPtr() {
    return Protected_GetByteStringParamPtr(static_cast<parameter_id_t>(BPSEC_BIB_HMAX_SHA2_SECURITY_PARAMETERS::WRAPPED_KEY));
}
