...
BOOST_AUTO_TEST_CASE(LtpFullTestCase)
{
    struct TestLtp {
        ...
        void ReceiveDataSegment() {
            ...
            {
                uint64_t extractedSessionOriginatorEngineId;
                BOOST_REQUIRE(Ltp::GetSessionOriginatorEngineIdFromLtpPacket(ltpHeaderPlusDataSegmentMetadata.data(), ltpHeaderPlusDataSegmentMetadata.size(), extractedSessionOriginatorEngineId));
                BOOST_REQUIRE_EQUAL(extractedSessionOriginatorEngineId, m_desired_sessionId.sessionOriginatorEngineId);
            }
            for (unsigned int i = 1; i <= 5; ++i) {
                std::string errorMessage;
