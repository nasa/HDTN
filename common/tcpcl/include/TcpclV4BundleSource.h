...
public:
    ...
    TCPCL_LIB_EXPORT bool ReadyToForward();
    TCPCL_LIB_EXPORT void SetOnSuccessfulAckCallback(const OnSuccessfulAckCallback_t & callback);
private:
    TCPCL_LIB_EXPORT void OnResolve(const boost::system::error_code & ec, boost::asio::ip::tcp::resolver::results_type results);
    TCPCL_LIB_EXPORT void OnConnect(const boost::system::error_code & ec);
    TCPCL_LIB_EXPORT void OnReconnectAfterOnConnectError_TimerExpired(const boost::system::error_code& e);
    TCPCL_LIB_EXPORT void StartTcpReceiveUnsecure();
    TCPCL_LIB_EXPORT void HandleTcpReceiveSomeUnsecure(const boost::system::error_code & error, std::size_t bytesTransferred);
#ifdef OPENSSL_SUPPORT_ENABLED
    TCPCL_LIB_EXPORT void StartTcpReceiveSecure();
    TCPCL_LIB_EXPORT void HandleTcpReceiveSomeSecure(const boost::system::error_code & error, std::size_t bytesTransferred);
    TCPCL_LIB_EXPORT void HandleSslHandshake(const boost::system::error_code & error);
#endif
    TCPCL_LIB_EXPORT void OnNeedToReconnectAfterShutdown_TimerExpired(const boost::system::error_code& e);
